...
export function ColorPicker({
  ...
  const [changeSource, setChangeSource] = useState<'hex' | 'external' | null>(null);
  ...
  const triggerChange = useCallback((next: HsvaColor, source: 'hex' | 'external' = 'external') => {
  ...
  const handleRgbChannelChange = (channel: "r"|"g"|"b", raw: string) => {
    ...
    const parsed = parseNumericChannel(raw, ranges[channel][0], ranges[channel][1]);
  ...
  const formatContrast = (ratio: number) => {
    switch (contrastFormat) {
      case "1:value": return `1:${ratio.toFixed(2)}`;
      case "value": return ratio.toFixed(2);
      default: return `${ratio.toFixed(2)}:1`;
    }
  };
  ...
  return (
    <div className={cn("flex flex-col select-none", isRTL && "rtl")} style={{
      ...
      borderWidth: containerBorderWidth, borderRadius: containerRadius, padding: containerPadding,
      gap: containerElementGap, direction: isRTL ? "rtl" : "ltr"
    }}>
      {/* Saturation Area */}
      <div ref={satContainerRef} onMouseDown={handleSatMouseDown} className="relative w-full cursor-crosshair" style={{
        height: saturationHeight, borderRadius: saturationRadius, borderStyle: "solid",
        borderColor: saturationBorderColor, borderWidth: saturationBorderWidth, overflow: "hidden",
      ...
      <CustomSlider
        ...
        trackHeight={typeof hueTrackHeight === 'number' ? `${hueTrackHeight}px` : hueTrackHeight}
        thumbWidth={typeof hueThumbSize === 'number' ? `${hueThumbSize}px` : hueThumbSize}
        ...
        trackFillBorderRadius={typeof hueTrackRadius === 'number' ? `${hueTrackRadius}px` : hueTrackRadius}
        colorTrackBackground={isRTL 
      ...
      {/* Controls Row */}
      <div className={cn("flex gap-2", modeDropdownFullWidth ? "flex-col" : "items-start")}>
        <div className={cn("flex w-full gap-2", modeDropdownFullWidth ? "flex-col" : "items-start")}>
          ...
          {showModeDropdown && (
            <div style={{width: modeDropdownFullWidth ? "100%" : modeDropdownWidth, flexShrink: 0}}>
              <FloatingLabelSelect
                value={format}
                onChange={(f) => {
                ...
                isRTL={isRTL}
                t={(key) => key === "hex" ? hexLabel : key === "rgb" ? rgbLabel : hslLabel}
      ...
      {showContrast && (
        ...
        >
          <div className="flex items-center" style={{ gap: contrastItemGap }}>
            {colorPreviewPosition === "contrast" && (
              <div className="inline-flex items-center justify-center select-none" style={{
                backgroundColor: hsvaToHex(hsva) || previewBgFallback,
                width: previewWidth, height: previewHeight,
                borderColor: previewBorderColor, borderWidth: previewBorderWidth,
                borderStyle: "solid", borderRadius: previewRadius,
                color: previewTextColor ?? autoPreviewTextColor,
                fontSize: previewFontSize, fontWeight: previewFontWeight,
              }}>
                {colorPreviewAreaText}
              </div>
            )}
            <div className="flex flex-col" style={{ gap: contrastLabelGap }}>
              <span style={{fontSize: contrastLabelSize, color: contrastLabelColor, fontWeight: contrastLabelWeight}}>
                {contrastLabel}
              </span>
              <span style={{fontSize: contrastValueSize, color: contrastValueColor, fontWeight: contrastValueWeight, fontFamily: "monospace"}}>
...
function Badge({
  ...
  const borderWidthStr = typeof badgeBorderWidth === 'number' ? `${badgeBorderWidth}px` : badgeBorderWidth;
  const borderRadiusStr = typeof badgeBorderRadius === 'number' ? `${badgeBorderRadius}px` : badgeBorderRadius;
  const fontSizeStr = typeof badgeFontSize === 'number' ? `${badgeFontSize}px` : badgeFontSize;
  const iconSizeStr = typeof badgeIconSize === 'number' ? `${badgeIconSize}px` : badgeIconSize;

  return (
    <div
      className="flex items-center gap-1 px-2 py-0.5 font-bold border select-none"
      style={{
    ...
    >
      {pass ? (
        <CheckCircle2 size={Number(iconSizeStr.replace('px', ''))} strokeWidth={badgeIconStrokeWidth} />
      ) : (
        <XCircle size={Number(iconSizeStr.replace('px', ''))} strokeWidth={badgeIconStrokeWidth} />
      )}
      {label}
    </div>
  );
...
export const FloatingLabelInput: React.FC<FloatingLabelInputProps> = ({
  value, onChange, label, isRTL, hasError = false,
  maxLength = 64, height, bg, borderColor, borderWidth, radius, fontSize,
  textColor, focusBorderColor, floatingLabelBg, floatingLabelTextColor,
  floatingLabelActiveTextColor, floatingLabelRadius, floatingLabelBorderColor,
  floatingLabelBorderWidth, errorOutlineColor,
}) => {
  ...
  const currentFocusBorderColor = hasError ? errorOutlineColor : focusBorderColor;
  ...
  return (
    ...
    >
      ...
      <label 
        className="floating-label"
        style={{
          color: isFocused ? floatingLabelActiveTextColor : floatingLabelTextColor,
          borderRadius: floatingLabelRadius,
          borderStyle: floatingLabelBorderColor ? "solid" : "none",
          borderColor: floatingLabelBorderColor,
          borderWidth: floatingLabelBorderWidth,
      ...
      {open && (
        <div
          ref={listRef}
          className={cn(
            "absolute z-50 mt-1 py-1 shadow-2xl border min-w-[120px] max-h-48 overflow-auto",
          ...
          {options.map((opt) => {
            ...
            return (
              <button
                ...
                className="w-full px-3 py-2 text-[11px] uppercase cursor-pointer flex items-center justify-between gap-2 transition-colors"
                style={{
                ...
                onMouseEnter={(e) => {
                  if (!active) e.currentTarget.style.backgroundColor = menuHoverBg;
                }}
                onMouseLeave={(e) => {
  ...
  return (
    ...
    >
      <input
        ...
        tabIndex={-1}
        value=""
        className="absolute inset-0 font-mono shadow-inner uppercase transition-all outline-none"
        style={{
          pointerEvents: "none",
          userSelect: "none",
          backgroundColor: bg,
...
export function CustomSlider({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight' || e.key === 'ArrowUp' || e.key === 'ArrowDown') {
      e.preventDefault();
      const direction = (e.key === 'ArrowRight' || e.key === 'ArrowUp' ? 1 : -1) * (isRTL ? -1 : 1);
  ...
  const thumbBorderColor = isDragging || isFocused ? colorThumbBorderActive : isHovered ? colorThumbBorderHover : colorThumbBorderDefault;
