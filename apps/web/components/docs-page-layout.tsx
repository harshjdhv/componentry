...
### Manual
${installDependencies ? `1. Install dependencies
\`\`\`bash
npm install ${installDependencies}
\`\`\`
` : ''}

${installSourceCode ? `${installDependencies ? "2" : "1"}. Copy source code
\`\`\`tsx
${installSourceCode}
\`\`\`
` : ''}
                        ...
                        <div className="space-y-6">
                            {installDependencies && (
                                <div className="space-y-3">
                                    ...
                                    <CodeBlock code={`npm install ${installDependencies}`} lang="bash" />
                                </div>
                            )}
                            {installSourceCode && (
                                <div className="space-y-3">
                                    <p className="text-base font-semibold">{installDependencies ? "2" : "1"}. Copy the source code</p>
                                    <p className="text-sm text-muted-foreground">
                                        Copy the following code to <code className="text-xs bg-muted px-1 py-0.5 rounded font-mono text-foreground">{installSourceFilename || `components/ui/${installPackageName}.tsx`}</code>
                                    </p>
                                    <CodeBlock
                    ...
                    <div className="space-y-10">
                        {examples.map((example, index) => (
                            <div key={index} className="space-y-4">
                                <h3 className="font-heading text-xl font-semibold tracking-tight">{example.title}</h3>
                                <div className="space-y-0">
                                    <ComponentPreview>
                                        {example.preview}
                                    </ComponentPreview>
                                    <CodeBlock code={example.code} lang="tsx" className="rounded-t-none" />
                                </div>
                            </div>
                        ))}
                    </div>
